...
namespace pos_asp_net_mvc.Controllers
{
    ...
    public class BrandController : Controller
    {
        ...
        public ActionResult DataTable()
        {
            ...
            var actions = "";
            actions += "&nbsp;<a href='javascript:void(0);' data-route='" + Url.Action("Details", "Brand", new { id = 0  })+"' class='btn btn-success btn-sm btn-show'><i class='fa fa-search'></i></a>";
            actions += "&nbsp;<a href='javascript:void(0);' data-route='" + Url.Action("Edit", "Brand", new { id = 0 }) + "' class='btn btn-info btn-sm btn-edit'><i class='fa fa-edit'></i></a>";
            actions += "&nbsp;<a href='javascript:void(0);' data-route='" + Url.Action("Delete", "Brand", new { id = 0 }) + "' class='btn btn-danger btn-sm btn-delete'><i class='fa fa-trash'></i></a>";


            var getData = (from items in db.Brands
                select new
                {
                    id = items.Id,
                    brand_name = items.Name,
                    brand_description = items.Description,
                    key_id = items.Id,
                    action = actions
                }
            );
            var totalRecord = db.Brands.Count();
            var getFilter = getData;

            if(!String.IsNullOrEmpty(searchValue) && searchValue.Length > 0)
            {
                getFilter = getData.Where(x => x.brand_name.Contains(searchValue) || x.brand_description.Contains(searchValue));
            }
            ...
            if (columnSortOrder.Equals("desc"))
            {
                ...
                {
                    case 0: getDataOrder = getFilter.OrderByDescending(x => x.brand_name); break;
                    case 1: getDataOrder = getFilter.OrderByDescending(x => x.brand_description); break;
            ...
            else
            {
                switch (int.Parse(columnIndex))
                {
                    case 0: getDataOrder = getFilter.OrderBy(x => x.brand_name); break;
                    case 1: getDataOrder = getFilter.OrderBy(x => x.brand_description); break;
        ...
        public async Task<ActionResult> Create([Bind(Include = "Id,Name,Description,CreatedAt,UpdatedAt")] Brand model)
        {
        ...
        public async Task<ActionResult> Details(int? id)
        {
            ...
            Brand model = await db.Brands.FindAsync(id);
            if (model == null)
            {
                return HttpNotFound();
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Edit([Bind(Include = "Id,Name,Description,CreatedAt,UpdatedAt")] Brand model)
        {
            if (ModelState.IsValid)
            {
