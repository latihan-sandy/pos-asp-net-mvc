...
namespace pos_asp_net_mvc.Controllers
{
    ...
    public class CategoryController : Controller
    {
        ...
        const String PAGE_NAME = "Category";
        ...
        public ActionResult DataTable()
        {
            ...
            var actions = "";
            actions += "&nbsp;<a href='javascript:void(0);' data-route='" + Url.Action("Details", "Category", new { id = 0 }) + "' class='btn btn-success btn-sm btn-show'><i class='fa fa-search'></i></a>";
            actions += "&nbsp;<a href='javascript:void(0);' data-route='" + Url.Action("Edit", "Category", new { id = 0 }) + "' class='btn btn-info btn-sm btn-edit'><i class='fa fa-edit'></i></a>";
            actions += "&nbsp;<a href='javascript:void(0);' data-route='" + Url.Action("Delete", "Category", new { id = 0 }) + "' class='btn btn-danger btn-sm btn-delete'><i class='fa fa-trash'></i></a>";


            var getData = (from items in db.Categories
                           select new
                           {
                               id = items.Id,
                               category_name = items.Name,
                               category_description = items.Description,
                               key_id = items.Id,
                               action = actions
                           }
            );
            ...
            {
                getFilter = getData.Where(x => x.category_name.Contains(searchValue) || x.category_description.Contains(searchValue));
            }
            ...
            if (columnSortOrder.Equals("desc"))
            {
                ...
                {
                    case 0: getDataOrder = getFilter.OrderByDescending(x => x.category_name); break;
                    case 1: getDataOrder = getFilter.OrderByDescending(x => x.category_description); break;
            ...
            else
            {
                switch (int.Parse(columnIndex))
                {
                    case 0: getDataOrder = getFilter.OrderBy(x => x.category_name); break;
                    case 1: getDataOrder = getFilter.OrderBy(x => x.category_description); break;
        ...
        public async Task<ActionResult> Create([Bind(Include = "Id,Name,Description,CreatedAt,UpdatedAt")] Category model)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Edit([Bind(Include = "Id,Name,Description,CreatedAt,UpdatedAt")] Category model)
        {
            if (ModelState.IsValid)
            {
